int* merge(int* firstArr, int lenArr1, int* secondArr, int lenArr2){
    ...
    if (firstArr[0]<firstArr[1]){
        ...
        while(firstIndex < lenArr1){
            mergeArr[mergeIndex]=firstArr[firstIndex];// Nếu mảng firstArr còn phần tử thì thêm vào cuối mảng mergeArr
            firstIndex++;
            mergeIndex++;
        }

        while(secondIndex < lenArr2){
            mergeArr[mergeIndex]=secondArr[secondIndex];// Nếu mảng secondArr còn phần tử thì thêm vào cuối mảng mergeArr
            secondIndex++;
            mergeIndex++;
        }
        return mergeArr;
